...
int         error_clean_command(t_command **command, t_deque **tokbuf_g,
                                t_deque **tokbuf_l)
{
    if (*command)
        free(*command);
...
int     match_command(t_command **command, t_deque **tokbuf_g)
{
    ...
    *command = init_command();
    ...
    {
        if (match_brace_group(&(*command)->brace_group, tokbuf_g) != PARSER_SUCCES)
            return (PARSER_ERROR);
    }
    else if (match_simple_command(&(*command)->simple_cmd, tokbuf_g) != PARSER_SUCCES)
        return (PARSER_ERROR);
